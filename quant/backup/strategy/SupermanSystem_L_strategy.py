            ...
            if current_position == 0:  # 空仓状态
                ...
                if data['vol'].iloc[i] > 0:
                    ...
                    if data['high'].iloc[i] >= profit_target:
                        signals.iloc[i] = [-1, 1, np.nan, max(data['open'].iloc[i], profit_target), 0]
                        current_position = 0
                    
                    # 止损
                    elif data['low'].iloc[i] <= stop_loss:
                        signals.iloc[i] = [-1, 1, np.nan, min(data['open'].iloc[i], stop_loss), 0]
                        current_position = 0
                    ...
                    # 反向出场
                        ...
                        signals.iloc[i] = [-1, 1, np.nan, min(data['open'].iloc[i], data['LL1'].iloc[i-1]), 0]
                        current_position = 0
